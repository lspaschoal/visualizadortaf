class TAF {
    constructor(taf_aisweb) {
        ...
        this.horarios = this.gerarHorarios(taf_aisweb.validade_inicial, taf_aisweb.validade_final);
    ...
    gerarHorarios(validade_inicial, validade_final) {
        ...
        while (current <= dataFinal) {
            let horario = {};
            horario.ddhhmm = `${String(current.getUTCDate()).padStart(2, "0")}${String(current.getUTCHours()).padStart(2, "0")}00`;
    ...
    inserirTEMPO(mensagem) {
        ...
        this.horarios.forEach(horario => {
            if (horario.ddhhmm >= inicio && horario.ddhhmm < fim) {
                ...
                if (condicao.visibilidade !== null) horario.visibilidade = condicao.visibilidade;
                if (condicao.teto !== null) horario.teto = condicao.teto;
                if (condicao.nuvens !== null) horario.nuvens = condicao.nuvens;
                if (condicao.tempo_presente !== null) horario.tempo_presente = condicao.tempo_presente;
    ...
    inserirFM(mensagem) {
        const condicao = this.lerCondicao(mensagem);
        let inicio = mensagem.match(/FM\d{6}\s/)[0].trim().substring(2);
        this.horarios.forEach((periodo, indice, arr) => {
            ...
            if (indice < arr.length - 1 && horario.ddhhmm < inicio && arr[indice+1].ddhhmm > inicio){
    ...
    lerCondicao(texto) {
        ...
        if (texto.match(/\s\d\d\d\d\s/)) condicao.visibilidade = (texto.match(/\s\d\d\d\d\s/)[0]).trim();
        if (texto.match(/(?:FEW|SCT|BKN|OVC)\d{3}(?:CB|TCU)?/)) condicao.nuvens = [...texto.matchAll(/(?:FEW|SCT|BKN|OVC)\d{3}(?:CB|TCU)?/g)].map(m => m[0]);
        if (texto.match(/NSC/)) condicao.nuvens = [];
        if (condicao.nuvens) condicao.nuvens.forEach(camada => {
        ...
        if (texto.match(/\b[+-]?(?:TSRA|RA|BR|FG|TS|VC[A-Z]{2})\b/)) condicao.tempo_presente = [...texto.matchAll(/\b[+-]?(?:TSRA|RA|BR|FG|TS|VC[A-Z]{2})\b/g)].map(m => m[0]);
        if (texto.match(/\d{5}KT|\d{5}G\d{2}KT|VRB\d{2}KT/)) condicao.vento = texto.match(/\d{5}KT|\d{5}G\d{2}KT|VRB\d{2}KT/)[0];
    ...
    gerarPeriodos(horarios){
        ...
        for (let i = 1; i < horarios.length - 1; i++) {
            periodos[periodos.length - 1].data_hora_fim = new Date(horarios[i].data_hora);
            if (horarios[i].visibilidade != periodos[periodos.length - 1].visibilidade ||
                horarios[i].teto != periodos[periodos.length - 1].teto ||
                horarios[i].vento != periodos[periodos.length - 1].vento ||
                !this.compararArrays(horarios[i].nuvens, periodos[periodos.length - 1].nuvens ||
                    !this.compararArrays(horarios[i].tempo_presente, periodos[periodos.length - 1].tempo_presente)
                )
            ) {
                periodos.push({
        ...
        periodos[periodos.length - 1].data_hora_fim =
            new Date(ultimoHorario.data_hora);
        return periodos;
    }
